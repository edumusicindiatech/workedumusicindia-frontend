...
function App() {
      ...
      try {
        ...
        if (profileRes.data.success) {
          ...
          if (error.response && error.response.status === 401) {
            ...
            console.warn("Network error during boot. User might be offline. Keeping session active.");
          }
        }
      } catch (error) {
        dispatch(logout());
  ...
  // 🚀 UPDATE: Sync AND Clear User ID in Native Storage
  useEffect(() => {
    if (Capacitor.isNativePlatform()) {
      const syncId = async () => {
        try {
  ...
  }, [user]); // 👈 Triggers whenever user logs in or out
