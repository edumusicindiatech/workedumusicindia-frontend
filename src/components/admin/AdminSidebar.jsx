...
const AdminSidebar = () => {
    ...
    // --- INITIALIZE REPORTS BADGE FROM LOCAL STORAGE ON MOUNT ---
    useEffect(() => {
        if (!location.pathname.includes('/reports')) {
            try {
                const storedDaily = JSON.parse(localStorage.getItem('adminUnreadDaily') || '{}');
                const storedEvents = JSON.parse(localStorage.getItem('adminUnreadEventsMap') || '{}');
                const totalUnread = Object.values(storedDaily).reduce((a, b) => a + b, 0) +
                    Object.values(storedEvents).reduce((a, b) => a + b, 0);
                setUnreadReportsCount(totalUnread);
            } catch (e) { }
        }
    }, []);
    ...
    useEffect(() => {
        ...
        // Hide badge instantly when admin navigates to Reports
        if (location.pathname.includes('/reports')) setUnreadReportsCount(0);
    }, [location.pathname]);
    ...
    useEffect(() => {
        const fetchUnreadReports = async () => {
            try {
                const res = await api.get('/admin/reports/unread-summary');
                if (res.data.success) {
                    const { dailyUnread, eventsUnread } = res.data.data;

                    // Overwrite local storage with the undisputed truth from the database
                    localStorage.setItem('adminUnreadDaily', JSON.stringify(dailyUnread));
                    localStorage.setItem('adminUnreadEventsMap', JSON.stringify(eventsUnread));

                    // Only set the global visual badge if we aren't currently viewing the Reports page
                    if (!location.pathname.includes('/reports')) {
                        const totalUnread = Object.values(dailyUnread).reduce((a, b) => a + b, 0) +
                            Object.values(eventsUnread).reduce((a, b) => a + b, 0);
                        setUnreadReportsCount(totalUnread);
                    }
                }
            } catch (e) {
                console.error("Failed to sync unread reports", e);
            }
        };
    ...
    // FETCH INITIAL UNREAD NOTIFICATIONS COUNT ON LOAD
    useEffect(() => {
        if (!user) return;
    ...
    // REAL-TIME SOCKET LISTENER FOR SYSTEM ALERTS & SOS
    useEffect(() => {
        if (!socket) return;
        ...
        // --- UPDATED: CAPTURE SPECIFIC IDs AND SAVE TO LOCAL STORAGE ---
        const handleNewDailyReport = (newReport) => {
            if (!pathnameRef.current.includes('/reports')) {
                setUnreadReportsCount(prev => prev + 1);

                // Track the specific employee so AdminReports sees it when it mounts
                if (newReport && newReport.teacher) {
                    const unreadDaily = JSON.parse(localStorage.getItem('adminUnreadDaily') || '{}');
                    unreadDaily[newReport.teacher] = (unreadDaily[newReport.teacher] || 0) + 1;
                    localStorage.setItem('adminUnreadDaily', JSON.stringify(unreadDaily));

                    // Push them to the top of the list
                    const activity = JSON.parse(localStorage.getItem('adminEmployeeActivity') || '{}');
                    activity[newReport.teacher] = Date.now();
                    localStorage.setItem('adminEmployeeActivity', JSON.stringify(activity));
                }
            }
        };
        ...
        const handleNewEvent = (newEvent) => {
            if (!pathnameRef.current.includes('/reports')) {
                ...
                // Track the specific school so AdminReports sees it when it mounts
                if (newEvent && newEvent.schoolName) {
                    const unreadEvents = JSON.parse(localStorage.getItem('adminUnreadEventsMap') || '{}');
                    unreadEvents[newEvent.schoolName] = (unreadEvents[newEvent.schoolName] || 0) + 1;
                    localStorage.setItem('adminUnreadEventsMap', JSON.stringify(unreadEvents));

                    // Push school to the top of the list
                    const activity = JSON.parse(localStorage.getItem('adminSchoolActivity') || '{}');
                    activity[newEvent.schoolName] = Date.now();
                    localStorage.setItem('adminSchoolActivity', JSON.stringify(activity));
                }
            }
        };
        ...
        <>
            <nav className="hidden 2xl:flex fixed top-0 w-full h-16 bg-card border-b border-border z-50 items-center justify-between px-6 shadow-sm">
                ...
                <div className="flex items-center justify-start flex-1 min-w-0 gap-1 overflow-x-auto px-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    ...
                    {/* --- REPORTS NAVLINK WITH GREEN BADGE --- */}
                    <NavLink to="/admin/reports" className={desktopNavClasses} title={t('sidebar.reports')}>
                        <div className="relative flex items-center justify-center">
                            <ClipboardCheck className="w-4.5 h-4.5" />
                            {unreadReportsCount > 0 && (
                                <span className="absolute -top-1.5 -right-2 flex h-4 w-4 items-center justify-center rounded-full bg-emerald-500 text-[9px] font-bold text-white shadow-sm border border-card animate-in zoom-in duration-300">
            ...
            <header className="2xl:hidden fixed top-0 left-0 w-full h-16 bg-card border-b border-border z-40 flex items-center justify-between px-4 shadow-sm">
                ...
                <div className="relative" ref={mobileMenuRef}>
                    <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="relative p-1 rounded-full hover:bg-muted transition-colors flex items-center justify-center w-10 h-10 overflow-hidden ring-2 ring-transparent focus:ring-primary/20 border border-border/50">
                        ...
                        {/* Mobile Header indicator if Reports are unread and menu is closed */}
                        {unreadReportsCount > 0 && !isMobileMenuOpen && (
                            <span className="absolute top-0 right-0 flex h-3 w-3 items-center justify-center rounded-full bg-emerald-500 border-2 border-card" />
                        )}
                    </button>
                    ...
                    <div className={`absolute top-12 right-0 w-56 bg-card border border-border rounded-2xl shadow-2xl p-2 z-50 origin-top-right transition-all duration-200 ease-out ${isMobileMenuOpen ? "opacity-100 scale-100 translate-y-0 pointer-events-auto" : "opacity-0 scale-95 -translate-y-2 pointer-events-none"}`}>
                        ...
                        {/* --- MOBILE DROPDOWN REPORTS WITH BADGE --- */}
                        <button className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground transition-colors" onClick={() => { setIsMobileMenuOpen(false); navigate('/admin/reports'); }}>
                            <div className="flex items-center gap-3"><ClipboardCheck className="w-4 h-4 text-primary" /> {t('sidebar.reports')}</div>
                            {unreadReportsCount > 0 && <span className="flex h-5 w-5 items-center justify-center rounded-full bg-emerald-500 text-[10px] font-bold text-white shadow-sm animate-in zoom-in">{unreadReportsCount > 99 ? '99+' : unreadReportsCount}</span>}
                        </button>
