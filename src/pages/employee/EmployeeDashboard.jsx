...
const EmployeeDashboard = () => {
    ...
    // Scenario 2 Check: Are ALL fetched assignments marked as a school holiday?
    const isFullHoliday = assignments.length > 0 && assignments.every(a => a.status === 'school_holiday');
    const hasWorkingShifts = assignments.some(a => a.status === 'pending' || a.status === 'checked_in');
    const showPalmTreeScreen = leaveData || (assignments.length > 0 && !hasWorkingShifts);
        ...
        <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8 pb-24 p-4 sm:p-6 lg:p-8 animate-in fade-in duration-700 relative mt-2 md:mt-0">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 sm:gap-6 pb-6 sm:pb-8 border-b border-border/50 relative z-20">
                <div className="space-y-1.5 sm:space-y-2">
                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary/60">
                    ...
                    <p className="text-muted-foreground font-bold uppercase tracking-widest flex items-center gap-1.5 sm:gap-2 text-[9px] sm:text-[10px] md:text-xs">
                        ...
                        {showPalmTreeScreen ? t('employee_dashboard.status_ooo', { defaultValue: 'OUT OF OFFICE' }) : t('employee_dashboard.status_route')}
                    </p>
                </div>

                <div className="flex items-center gap-2 sm:gap-3 w-full md:w-auto">
                    {(assignments.length === 0 || showPalmTreeScreen) && (
                        <Button
                    ...
                    {!showPalmTreeScreen && assignments.length > 0 && (
                        <>
                            <Button
                                variant="outline"
            ...
            <div className="mt-6 sm:mt-8 space-y-5 sm:space-y-6">
                {showPalmTreeScreen ? (
                    <div className="bg-card border border-border/60 rounded-3xl sm:rounded-[3rem] p-8 sm:p-10 md:p-16 mt-4 shadow-xl shadow-sky-500/5 dark:shadow-none text-center flex flex-col items-center relative overflow-hidden group transition-all duration-500 animate-in zoom-in-95">
                        <div className="absolute top-0 left-0 w-full h-1.5 sm:h-2 bg-gradient-to-r from-sky-400 via-blue-500 to-indigo-500" />
                        ...
                        <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-foreground mb-2.5 sm:mb-3 tracking-tight uppercase italic relative z-10">
                            {leaveData ? t('employee_dashboard.vacation.title') : t('employee_dashboard.vacation.school_closed', { defaultValue: 'School Holiday!' })}
                        </h2>
                        <p className="text-muted-foreground font-medium mb-6 sm:mb-8 max-w-md text-sm sm:text-base md:text-lg leading-relaxed relative z-10 px-4">
                            {leaveData
                                ...
                                : t('employee_dashboard.vacation.all_closed_desc', { defaultValue: `Enjoy your day off, ${user?.name?.split(' ')[0]}! All your assigned schools are closed today for the holiday.` })
                            }
                        </p>
                    ...
                    <div className="bg-card border border-border/60 rounded-3xl sm:rounded-[3rem] p-8 sm:p-10 md:p-16 mt-4 shadow-xl shadow-emerald-500/5 dark:shadow-none text-center flex flex-col items-center relative overflow-hidden group transition-all duration-500 animate-in zoom-in-95">
                        <div className="absolute top-0 left-0 w-full h-1.5 sm:h-2 bg-gradient-to-r from-emerald-400 via-teal-500 to-emerald-600" />
                    ...
                    <div className="space-y-5 sm:space-y-6 lg:space-y-8">
                        {assignments.map((visit) => {
                            // --- NEW SCENARIO 1 CHECK ---
                            const isSchoolHoliday = visit.status === 'school_holiday';
                            ...
                            return (
                                <div key={visit.id} className={`group relative bg-card rounded-4xl sm:rounded-[2.5rem] p-5 sm:p-6 md:p-8 transition-all duration-300 border flex flex-col h-full overflow-hidden ${isSchoolHoliday ? 'opacity-60 grayscale-[50%] bg-muted/20' : isActive ? 'border-emerald-500/50 shadow-xl shadow-emerald-500/10 dark:bg-emerald-950/10 scale-[1.01]' : 'border-border/60 hover:shadow-xl hover:shadow-slate-200/50 hover:border-primary/40 active:scale-[0.99] dark:hover:shadow-none'}`}>
                                    ...
                                    <div className="flex flex-col lg:flex-row justify-between gap-5 sm:gap-6 md:gap-8 relative z-10">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex flex-wrap items-center gap-2.5 sm:gap-3 mb-4 sm:mb-5">
                                                ...
                                                {isSchoolHoliday && (
                                                    <span className="px-3 sm:px-4 py-1 sm:py-1.5 bg-amber-500/10 text-amber-600 dark:text-amber-500 border border-amber-500/20 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] md:text-xs font-black uppercase tracking-widest flex items-center gap-1.5 sm:gap-2 shadow-sm">
                                                        🌴 Holiday: {visit.holidayTitle}
                                                    </span>
                                                )}
                                        ...
                                        <div className={`p-4 sm:p-5 md:p-6 rounded-2xl sm:rounded-3xl border text-left lg:text-right min-w-40 sm:min-w-50 flex flex-row lg:flex-col justify-between items-center lg:items-end shrink-0 shadow-sm transition-colors ${isActive ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-muted/30 border-border/60 group-hover:border-primary/20 group-hover:bg-muted/50'}`}>
                                            <div>
                                                ...
                                                <p className={`text-lg sm:text-xl md:text-2xl font-black tracking-tight ${isActive ? 'text-emerald-600 dark:text-emerald-400' : isSchoolHoliday ? 'text-muted-foreground line-through decoration-2 decoration-muted-foreground/50' : 'text-foreground'}`}>
                                                    {formatTo12Hour(visit.startTime)} <span className="opacity-50 font-medium px-0.5">-</span> {formatTo12Hour(visit.endTime)}
                                                </p>
                                            </div>
                                            {isPending && !isSchoolHoliday && (
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center gap-2.5 sm:gap-3 mt-6 sm:mt-8 pt-5 sm:pt-6 border-t border-border/50 relative z-10">
                                        ...
                                        {/* HIDE Check-in/Check-out buttons if it's a School Holiday */}
                                        {!isSchoolHoliday && (
