        ...
        <div className={`fixed inset-0 z-200 flex items-end md:items-center justify-center bg-black/60 transition-all duration-300 md:p-4 ${isClosing ? 'opacity-0' : 'opacity-100 backdrop-blur-sm animate-in fade-in'}`} onClick={handleClose}>
            ...
            >
                ...
                <div className="p-6 md:p-8 text-center pt-6 md:pt-10">
                    ...
                    <h3 className="text-2xl font-extrabold text-foreground mb-2 tracking-tight">Remove Picture?</h3>
                ...
                <div className="bg-muted/10 p-5 border-t border-border/50 flex flex-col gap-2 rounded-b-3xl pb-safe">
                    ...
                    >
                        {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Yes, Remove Photo"}
                    </Button>
                    <button 
                        onClick={handleClose} 
                        disabled={loading} 
...
const MyProfile = () => {
    ...
    const handlePasswordChange = async (newPassword) => {
        ...
        const toastId = toast.loading(t('my_profile.toasts.updating', 'Updating password...'));
        try {
            await api.put('/employee/profile/password', { newPassword });
            setIsPasswordModalOpen(false);
            toast.success(t('my_profile.toasts.success', 'Password updated successfully!'), { id: toastId });
        } catch (error) {
            toast.error(error.response?.data?.message || t('my_profile.toasts.error', 'Update failed'), { id: toastId });
        } finally {
    ...
    const handleAvatarUpload = async (e) => {
        ...
        if (!file) return;
        if (file.size > 5 * 1024 * 1024) return toast.error(t('my_profile.toasts.file_too_large', 'File must be under 5MB'));
        
        setIsUploadingAvatar(true);
        const toastId = toast.loading(t('my_profile.toasts.uploading_avatar', 'Uploading new photo...'));
        try {
            ...
            toast.success(t('my_profile.toasts.avatar_updated', 'Profile picture updated!'), { id: toastId });
            dispatch(updateProfilePicture(publicUrl));
            fetchFreshData();
        } catch (error) {
            toast.error(t('my_profile.toasts.upload_failed', 'Failed to upload photo'), { id: toastId });
        } finally {
    ...
    const handleDeleteAvatar = async () => {
        ...
        const toastId = toast.loading(t('my_profile.toasts.removing_avatar', 'Removing photo...'));
        try {
            const res = await api.delete('/employee/profile-picture');
            if (res.data.success) {
                toast.success(t('my_profile.toasts.avatar_removed', 'Photo removed successfully'), { id: toastId });
                dispatch(updateProfilePicture(null));
                fetchFreshData();
                setIsDeleteModalOpen(false);
            }
        } catch (error) {
            toast.error(t('my_profile.toasts.remove_failed', 'Failed to remove photo'), { id: toastId });
        } finally {
            ...
            <div className="bg-card rounded-[2.5rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-border/60 p-6 md:p-10 relative overflow-hidden group">
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-end justify-between gap-6 mb-10 pb-10 border-b border-border/50 relative z-10">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="flex flex-wrap justify-center sm:justify-start gap-2">
                                <span className="text-xs font-black bg-primary/10 text-primary border border-primary/20 px-4 py-1.5 rounded-full uppercase tracking-widest">
                                    {localUser.designation || t('my_profile.employee_fallback', 'Employee')}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <span>{t('my_profile.btn_edit_password', 'Security Settings')}</span>
                    </Button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                    <div className="space-y-5">
                        <Label className="text-[11px] font-black text-muted-foreground uppercase tracking-[0.2em] flex items-center gap-2 ml-1">
                            <User className="w-4 h-4 text-primary/60" /> {t('my_profile.contact_info', 'Contact Intelligence')}
                        </Label>
                        <div className="space-y-3">
                            <ProfileDetailItem icon={<Fingerprint className="text-indigo-500" />} label={t('my_profile.label_id', 'Employee ID')} value={localUser.employeeId} />
                            <ProfileDetailItem icon={<Mail className="text-blue-500" />} label={t('my_profile.label_email', 'Email Address')} value={localUser.email} />
                            <ProfileDetailItem icon={<Phone className="text-emerald-500" />} label={t('my_profile.label_phone', 'Mobile Number')} value={localUser.mobile || t('my_profile.not_provided', 'Not Linked')} />
                        </div>
                    </div>

                    <div className="space-y-5">
                        <Label className="text-[11px] font-black text-muted-foreground uppercase tracking-[0.2em] flex items-center gap-2 ml-1">
                            <MapPin className="w-4 h-4 text-primary/60" /> {t('my_profile.assignment_details', 'Logistics & Access')}
                        </Label>
                        <div className="space-y-3 h-full flex flex-col">
                            <ProfileDetailItem icon={<MapPin className="text-amber-500" />} label={t('my_profile.label_zone', 'Operating Zone')} value={allottedLocation} />
                            <div className="flex items-start gap-4 p-5 rounded-3xl bg-muted/30 border border-border/50 transition-all hover:border-primary/20 flex-1 group/item">
                                ...
                                <div className="min-w-0 flex-1">
                                    <p className="text-[10px] text-muted-foreground font-black uppercase tracking-wider mb-2.5">{t('my_profile.label_locations', 'Assigned Facilities')}</p>
