            ...
            {tasks.length === 0 ? (
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 sm:gap-8 mt-6 sm:mt-8">
                    {tasks.map((task) => {
                        ...
                        // --- FIX: Derive Dates cleanly for the UI ---
                        const startDateStr = task.startDate ? toISTDateString(task.startDate) : null;
                        const endDateStr = task.endDate ? toISTDateString(task.endDate) : "Ongoing";
                        ...
                        return (
                            ...
                            >
                                ...
                                {/* Scheduling Info */}
                                <div className={`grid grid-cols-1 gap-4 mb-5 sm:mb-6 p-4 sm:p-5 rounded-2xl sm:rounded-3xl border min-w-0 w-full ${isResolved ? 'bg-muted/10 border-border/30' : 'bg-muted/20 border-border/50'}`}>
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border/50">
                                        <div className="space-y-1.5 min-w-0">
                                            <div className="flex items-center gap-2 text-muted-foreground text-[9px] sm:text-[10px] font-black uppercase tracking-widest">
                                                <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary/70 shrink-0 pointer-events-none" /> {t('manage_task.start_date', 'Start Date')}
                                            </div>
                                            <p className={`font-black text-xs sm:text-sm uppercase tracking-widest truncate ${isResolved ? 'text-muted-foreground' : 'text-foreground'}`}>
                                                {startDateStr || 'Not Set'}
                                            </p>
                                        </div>
                                        <div className="space-y-1.5 min-w-0">
                                            <div className="flex items-center gap-2 text-muted-foreground text-[9px] sm:text-[10px] font-black uppercase tracking-widest">
                                                <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary/70 shrink-0 pointer-events-none" /> {t('manage_task.end_date', 'End Date')}
                                            </div>
                                            <p className={`font-black text-xs sm:text-sm uppercase tracking-widest truncate ${isResolved ? 'text-muted-foreground' : 'text-foreground'}`}>
                                                {endDateStr}
                                            </p>
                                        ...
                                        <div className="space-y-2.5 pt-3 sm:pt-0 border-t sm:border-t-0 sm:border-l border-border/50 sm:pl-5 min-w-0">
                                            ...
                                            <p className={`font-black text-xs sm:text-sm uppercase tracking-widest truncate ${isResolved ? 'text-muted-foreground' : 'text-foreground'}`}>
                                                {task.startTime && task.endTime
                                                    ? `${convertTo12HourFormat(task.startTime)} - ${convertTo12HourFormat(task.endTime)}`
