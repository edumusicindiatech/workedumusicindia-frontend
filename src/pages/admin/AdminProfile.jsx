...
const AdminProfile = () => {
    ...
    const handlePasswordChange = async (newPassword) => {
        ...
        const toastId = toast.loading(t('admin_profile.toasts.updating'));
        try {
            await api.put('/admin/profile/password', { newPassword });
            setIsPasswordModalOpen(false);
            toast.success(t('admin_profile.toasts.success'), { id: toastId });
        } catch (error) {
            toast.error(error.response?.data?.message || t('admin_profile.toasts.error'), { id: toastId });
        } finally {
    ...
    const handleAvatarUpload = async (e) => {
        ...
        try {
            ...
            toast.success(t('admin_profile.toasts.avatar_updated'), { id: toastId });
            dispatch(updateProfilePicture(publicUrl));
            fetchFreshData();
        } catch (error) {
            toast.error(t('admin_profile.toasts.upload_failed'), { id: toastId });
        } finally {
    ...
    const handleDeleteAvatar = async () => {
        ...
        const toastId = toast.loading(t('admin_profile.toasts.removing_avatar'));
        try {
            const res = await api.delete('/admin/profile-picture');
            if (res.data.success) {
                toast.success(t('admin_profile.toasts.avatar_removed'), { id: toastId });
                dispatch(updateProfilePicture(null));
                fetchFreshData();
                setIsDeleteModalOpen(false);
            }
        } catch (error) {
            toast.error(t('admin_profile.toasts.remove_failed'), { id: toastId });
        } finally {
