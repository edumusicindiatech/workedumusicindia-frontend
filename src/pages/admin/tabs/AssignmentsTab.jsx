...
// --- Bulletproof chronological day sorter ---
const sortDaysChronologically = (daysArray) => {
    ...
    return [...daysArray].sort((a, b) => {
        // Strip out any hidden spaces and force lowercase for a perfect match
        const cleanA = (a || "").toString().trim().toLowerCase();
        const cleanB = (b || "").toString().trim().toLowerCase();

        const orderA = dayOrder[cleanA] || 8;
        const orderB = dayOrder[cleanB] || 8;
                                        ...
                                        <p className="text-sm sm:text-base font-bold text-foreground flex items-center flex-wrap gap-2">
                                            <span>
                                                {assignment.allowedDays && assignment.allowedDays.length > 0
                                                    ? sortDaysChronologically(assignment.allowedDays).join(", ")
                                                    : "N/A"}
                                            </span>
                                        </p>
                                    </div>
