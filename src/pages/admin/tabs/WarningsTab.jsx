        ...
        <div className="bg-card rounded-xl shadow-card border border-border overflow-hidden">
            ...
            <div className="p-4 sm:p-6 border-b border-border flex justify-between items-center bg-muted/10">
                <h3 className="text-base sm:text-lg font-bold flex items-center gap-2">
                    <AlertTriangle className="w-5 h-5 text-destructive shrink-0" /> {t('warnings_tab.title')}
                </h3>
            ...
            <div className="p-0">
                        ...
                        >
                            <div className="min-w-0 flex-1 space-y-2.5">
                                ...
                                <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                                    ...
                                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border shrink-0 ${getWarningColor(level)}`}>
                                ...
                                <div className="flex flex-wrap items-center gap-4 text-xs font-medium text-muted-foreground pt-1">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        <User className="w-3.5 h-3.5 opacity-70" /> {t('warnings_tab.issued_by')}: {w.issuedBy?.name || t('warnings_tab.admin_default')}
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {(!warningsList || warningsList.length === 0) && (
                    <div className="p-8 sm:p-12 text-center text-muted-foreground flex flex-col items-center">
                        ...
                        <p className="font-medium">{t('warnings_tab.empty_title')}</p>
                        <p className="text-sm mt-1 max-w-xs mx-auto">{t('warnings_tab.empty_desc')}</p>
                    </div>
                )}
            </div>
