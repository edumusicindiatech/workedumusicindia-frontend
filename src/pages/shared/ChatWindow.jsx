...
import {
    ...
    Video, ChevronDown, ShieldCheck, History // 🟢 NEW: Added History icon
} from "lucide-react";
...
// 🟢 NEW: Import your newly created Audit Modal (Adjust this path if necessary!)
import ChatAuditModal from "../../modals/admin/ChatAuditModal";
...
const ChatWindow = ({
    ...
    const [manageContactsSearchQuery, setManageContactsSearchQuery] = useState("");
    const [manageContactsSelectedUsers, setManageContactsSelectedUsers] = useState([]);
    ...
    useEffect(() => {
        // Invisibly reset zoom scale when the image changes
        if (fullscreenMedia) imageScale.current = 1;
    }, [fullscreenMedia]);

    const isGroupChat = activeChat?.members !== undefined || activeChat?.isGroup;
    const isOnline = (id) => onlineUsers.includes(id?.toString());

    // Admin Role Checks for Whitelist Feature
    const isCurrentUserAdmin = user?.role === 'Admin' || user?.role === 'SuperAdmin';
    const isActiveChatEmployee = !isGroupChat && activeChat?.role === 'Employee';
    const canManageContacts = isCurrentUserAdmin && isActiveChatEmployee;
    ...
    }, [socket, activeChat, setConversations, setActiveChat]);
    ...
    const handleOpenManageContacts = async () => {
        const targetId = activeChat._id || activeChat.id;
        const tid = toast.loading(t('toast.loading') || "Loading...");
        try {
            const res = await api.get(`/admin/employees/${targetId}/whitelist`);
            if (res.data.success) {
                setManageContactsSelectedUsers(res.data.data || []);
                setShowManageContactsModal(true);
                toast.dismiss(tid);
            }
        } catch (error) {
            toast.error(t('toast.error') || "Failed to load whitelist", { id: tid });
        }
    };

    const handleSaveWhitelist = async () => {
        const targetId = activeChat._id || activeChat.id;
        const tid = toast.loading(t('toast.saving') || "Saving...");
        try {
            await api.put(`/admin/employees/${targetId}/whitelist`, {
                allowedContacts: manageContactsSelectedUsers
            });
            toast.success(t('toast.success') || "Allowed contacts updated!", { id: tid });
            setShowManageContactsModal(false);
            setManageContactsSearchQuery("");
        } catch (error) {
            toast.error(t('toast.error') || "Failed to update whitelist", { id: tid });
        }
    };
    ...
    const handleMediaUpload = async (e, options = {}) => {
        ...
        try {
            for (let i = 0; i < files.length; i++) {
                ...
                moveToTop(activeChat._id || activeChat.id, optimisticPayload);
    ...
    const downloadToLocal = async (url) => {
        ...
        try {
            // 1. Fetch the secure download URL from your backend
            const response = await api.post('/chat/generate-download-url', { fileUrl: url });
            ...
            // 3. Extract filename and check if it's a document
            const fileName = url.split('/').pop().split('?')[0] || `download_${Date.now()}`;
            const isDoc = fileName.match(/\.(pdf|doc|docx|xls|xlsx|ppt|pptx|txt|csv)$/i);
            ...
            if (Capacitor.isNativePlatform()) {
                ...
                reader.onloadend = async () => {
                    try {
                        ...
                        // Just show success, DO NOT open the Share sheet
                        toast.success(t('toast.download_success') || "File downloaded successfully!", { id: tid });

                        // If it's a document, immediately open it in the native viewer
                        if (isDoc) {
                            try {
                                ...
                                console.log("FileOpener failed or not installed", openErr);
                            }
                        }

                    } catch (err) {
                        console.error("Native file write error:", err);
                        toast.error(t('toast.download_fail') || "Failed to save file.", { id: tid });
                    }
                };
            } else {
                // --- WEB BROWSER PATH ---
            ...
            // Fallback: Open URL in browser if fetch fails
    ...
    const handleBatchForward = async () => {
        ...
        try {
            for (const recipientId of forwardSelectedUsers) {
                ...
                let lastPayload = null; // Store reference to pass to moveToTop later
                ...
                for (const msg of msgs) {
                    const payload = {
                        ...
                        groupId: isTargetGroup ? recipientId : null, // Added missing groupId
                        text: msg.text || "",
                    ...
                    lastPayload = payload; // Update reference on each iteration
                ...
                // Now this executes safely because lastPayload is accessible in this scope
                if (lastPayload) {
                    moveToTop(recipientId, lastPayload);
                }
            }
            toast.success(t('toast.forward_success') || "Forwarded", { id: tid });
        } catch (error) {
            console.error("Forwarding Error:", error);
            toast.error(t('toast.forward_fail') || "Forward failed", { id: tid });
        } finally {
    ...
    const onMediaTouchStart = (e) => {
        // Ignore swipe if using two fingers (pinching) or if zoomed in
        if (e.touches.length > 1 || imageScale.current > 1.05) {
            setTouchStartX(null);
            return;
        }
        setTouchEndX(null);
    ...
    const onMediaTouchMove = (e) => {
        if (e.touches.length > 1 || imageScale.current > 1.05) return;
        setTouchEndX(e.touches[0].clientX);
    };

    const onMediaTouchEnd = () => {
        if (!touchStartX || !touchEndX || !fullscreenMedia || imageScale.current > 1.05) return;
        const dist = touchStartX - touchEndX;

        const i = chatMediaFiles.findIndex(m => String(m._id || m.id) === String(fullscreenMedia._id || fullscreenMedia.id));

        if (dist > 75 && i < chatMediaFiles.length - 1) {
            setSlideDirection("slide-in-from-right-16");
            setFullscreenMedia(chatMediaFiles[i + 1]);
        } else if (dist < -75 && i > 0) {
    ...
    // Modified to not close menu incorrectly on modal opens
    const handleChatAction = async (action) => { setShowTopMenu(false); if (action !== 'whitelist' && action !== 'audit') setSharedContentView(action); };
    ...
    return (
        <div className={`flex-1 flex flex-col h-full w-full max-w-full relative overflow-hidden transition-all duration-300 animate-in slide-in-from-right-4 md:animate-none`}>
            ...
            {fullscreenMedia && !showForwardDialog && (
                <div className="absolute inset-0 z-100 w-full h-full bg-[#0b141a] flex flex-col animate-in fade-in duration-200 ease-out overflow-hidden">
                    ...
                    <div
                        ...
                        onTouchStartCapture={onMediaTouchStart}
                        onTouchMoveCapture={onMediaTouchMove}
                        onTouchEndCapture={onMediaTouchEnd}
                    >
                        <div key={fullscreenMedia._id || fullscreenMedia.id} className={`w-full h-full flex items-center justify-center animate-in fade-in ${slideDirection} duration-300 ease-out`}>
                            {fullscreenMedia.mediaType === 'image' && (
                                <TransformWrapper
                                    ...
                                    onTransformed={(ref) => {
                                        // Invisibly track scale without causing a laggy re-render
                                        imageScale.current = ref.state.scale;
                                    }}
                                >
                                    {({ scale }) => (
                        ...
                        <div className="p-4 md:p-5 border-t border-border/50 flex items-center justify-between bg-muted/20">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {createGroupSelectedUsers.length > 0 && (
                                    <button
                                        onClick={() => { /* Handled in Sidebar or parent if passed correctly, but we leave it here for the modal */ }}
                                        className="px-5 py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold rounded-xl transition-colors shadow-sm"
                                    >
                                        {t('modal.add_member.add_btn') || "Add Selected"}
                                    </button>
                                )}
                            </div>
                        </div>
            ...
            {/* 🟢 NEW: MANAGE CONTACTS (WHITELIST) MODAL */}
            {showManageContactsModal && (
                <div className="absolute inset-0 z-100 w-full h-full bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200 overflow-hidden">
                    <div className="w-full max-w-sm md:max-w-md bg-card border border-border shadow-2xl rounded-3xl overflow-hidden flex flex-col max-h-[85%] animate-in zoom-in-95 duration-200 ease-out">
                        <div className="p-4 md:p-5 border-b border-border/50 bg-muted/20">
                            <h3 className="text-lg font-bold text-foreground mb-1 whitespace-nowrap">{t('modal.whitelist.title') || "Manage Allowed Contacts"}</h3>
                            <p className="text-xs text-muted-foreground mb-4">Select who {activeChat?.name} is allowed to chat with.</p>
                            <div className="relative">
                                <Search className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground" />
                                <input type="text" placeholder={t('modal.whitelist.search') || "Search employees..."} value={manageContactsSearchQuery} onChange={(e) => setManageContactsSearchQuery(e.target.value)} className="w-full bg-background border border-border/60 rounded-xl pl-10 pr-4 py-2.5 text-[15px] focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all shadow-sm" autoFocus />
                            </div>
                        </div>

                        <div className="flex-1 overflow-y-auto p-2 custom-scrollbar">
                            {/* Filter out Groups and the active user themselves */}
                            {conversations.filter(c => !c.isGroup && !c.members && String(c._id || c.id) !== String(activeChat._id || activeChat.id) && c.name.toLowerCase().includes(manageContactsSearchQuery.toLowerCase())).map(user => {
                                const isSelected = manageContactsSelectedUsers.includes(user._id || user.id);
                                return (
                                    <label key={user._id || user.id} className={`flex items-center gap-3.5 p-3 rounded-xl cursor-pointer transition-colors ${isSelected ? 'bg-primary/5 dark:bg-primary/10' : 'hover:bg-muted/50'}`}>
                                        <div className="relative flex items-center justify-center">
                                            <input type="checkbox" checked={isSelected} onChange={(e) => { const id = user._id || user.id; if (e.target.checked) setManageContactsSelectedUsers(prev => [...prev, id]); else setManageContactsSelectedUsers(prev => prev.filter(userId => userId !== id)); }} className="w-5 h-5 rounded-md border-border text-primary focus:ring-primary focus:ring-offset-background cursor-pointer" />
                                        </div>
                                        <div className="shrink-0">
                                            {user.profilePicture ? <img src={user.profilePicture} alt={user.name} className="w-10 h-10 rounded-full object-cover" /> : <div className="w-10 h-10 rounded-full bg-linear-to-br from-primary/10 to-primary/20 flex items-center justify-center text-primary font-bold">{user.name?.charAt(0)}</div>}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <span className="font-semibold text-[15px] text-foreground truncate block">{user.name}</span>
                                            <span className="text-[12px] text-muted-foreground truncate block">{user.role || 'Employee'}</span>
                                        </div>
                                    </label>
                                );
                            })}
                        ...
                        <div className="p-4 md:p-5 border-t border-border/50 flex items-center justify-between bg-muted/20">
                            <span className="text-sm font-medium text-muted-foreground">{manageContactsSelectedUsers.length} allowed</span>
                            <div className="flex items-center gap-2">
                                <button onClick={() => { setShowManageContactsModal(false); setManageContactsSearchQuery(""); }} className="px-4 py-2.5 text-[14px] font-semibold text-muted-foreground hover:text-foreground hover:bg-muted rounded-xl transition-colors">{t('modal.common.cancel') || "Cancel"}</button>
                                <button onClick={handleSaveWhitelist} className="px-5 py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold rounded-xl transition-transform active:scale-95 shadow-md">
                                    {t('modal.common.save') || "Save"}
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    {isSelectionMode ? (
                        ...
                        <div className="h-14 sm:h-16 md:h-17.5 px-1.5 sm:px-5 bg-card dark:bg-[#13151A] border-b border-border/40 flex items-center justify-between shrink-0 z-20 sticky top-0 w-full">
                            ...
                            <div className="flex items-center justify-end shrink-0 gap-0.5 sm:gap-1 relative" ref={topMenuRef}>
                                ...
                                {showTopMenu && (
                                    <div className="absolute top-12 right-0 min-w-55 bg-card border border-border shadow-2xl rounded-xl p-1.5 flex flex-col gap-1 z-30 animate-in zoom-in-95 duration-200 ease-out origin-top-right" onClick={e => e.stopPropagation()}>
                                        <button onClick={() => handleChatAction('media')} className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-muted rounded-lg text-sm whitespace-nowrap font-medium transition-colors"><ImageIcon className="w-4 h-4 text-blue-500" /> {t('chat_window.top_menu.media') || "Media"}</button>
                                        <button onClick={() => handleChatAction('docs')} className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-muted rounded-lg text-sm whitespace-nowrap font-medium transition-colors"><FileText className="w-4 h-4 text-amber-500" /> {t('chat_window.top_menu.docs') || "Docs"}</button>
                                        <button onClick={() => handleChatAction('links')} className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-muted rounded-lg text-sm whitespace-nowrap font-medium transition-colors"><LinkIcon className="w-4 h-4 text-emerald-500" /> {t('chat_window.top_menu.links') || "Links"}</button>

                                        {/* 🟢 NEW: Admin 'Manage Contacts' Button */}
                                        {canManageContacts && (
                                            <>
                                                <button onClick={() => { handleChatAction('whitelist'); handleOpenManageContacts(); }} className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-muted rounded-lg text-sm whitespace-nowrap font-medium transition-colors">
                                                    <ShieldCheck className="w-4 h-4 text-purple-500" /> {t('chat_window.top_menu.manage_contacts') || "Manage Contacts"}
                                                </button>

                                                {/* 🟢 NEW: Admin 'Show Chats' (Audit) Button */}
                                                <button onClick={() => { handleChatAction('audit'); setShowAuditModal(true); }} className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-muted rounded-lg text-sm whitespace-nowrap font-medium transition-colors">
                                                    <History className="w-4 h-4 text-orange-500" /> {t('chat_window.top_menu.show_chats') || "Show Chats"}
                                                </button>
                                            </>
                                        )}

                                        <button onClick={() => setShowClearChatModal(true)} className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-rose-500/10 rounded-lg text-sm whitespace-nowrap font-medium text-rose-500 transition-colors"><Trash className="w-4 h-4" /> {t('chat_window.top_menu.clear_chat') || "Clear chat"}</button>
                                    </div>
                                )}
                            </div>
                        </div>
