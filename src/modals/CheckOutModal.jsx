        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300" onClick={onClose}>
            <div className="bg-card w-full max-w-md rounded-2xl shadow-2xl border border-border flex flex-col animate-in zoom-in-95 fade-in duration-300 p-6" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex items-center gap-3 mb-6 border-b border-border pb-4">
                    ...
                    <div>
                        <h2 className="text-lg font-bold">Check Out</h2>
                ...
                <div className="space-y-5">
                    ...
                    {overtimeMinutes > 0 && (
                        <div className="space-y-2">
                            <label className="text-sm font-semibold text-amber-500 flex items-center gap-2">
                                <Clock className="w-4 h-4 shrink-0" /> You are {overtimeMinutes} minutes into overtime.
                            </label>
                            <p className="text-xs text-muted-foreground mb-2">Please provide an optional reason for the extended stay.</p>
                            <textarea
                                value={overtimeReason}
                                onChange={(e) => setOvertimeReason(e.target.value)}
                                placeholder="Meeting ran long, extra students, delayed start..."
                    ...
                    {!overtimeMinutes && (
                        <p className="text-sm text-muted-foreground">Are you sure you want to complete this visit and check out?</p>
                    )}
                    ...
                    <div className="flex gap-3 pt-4 border-t border-border">
                        ...
                        >
                            {actionLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Confirm Check Out"}
                        </Button>
                    </div>
                </div>
            </div>
