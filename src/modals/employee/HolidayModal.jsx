            ...
            <div className="bg-card w-full max-w-md rounded-2xl shadow-2xl border border-border flex flex-col animate-in zoom-in-95 fade-in duration-300 p-6" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex items-center justify-between gap-3 mb-6 border-b border-border pb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-amber-500">{t('holiday_modal.title')}</h2>
                            <p className="text-xs text-muted-foreground truncate max-w-50">
                                {target === 'ALL' ? t('holiday_modal.entire_day') : `${target.schoolName}`}
                            </p>
                        </div>
                    </div>
                ...
                <div className="space-y-5">
                    <div className="space-y-2">
                        <label className="text-sm font-semibold flex items-center gap-2 text-foreground">
                            {t('holiday_modal.reason_label')} <span className="text-xs font-normal opacity-60 text-muted-foreground">{t('holiday_modal.required')}</span>
                        </label>
                        <textarea
                            value={holidayReason}
                            onChange={(e) => setHolidayReason(e.target.value)}
                            placeholder={t('holiday_modal.placeholder')}
                    ...
                    <div className="flex gap-3 pt-4 border-t border-border/50">
                        <Button variant="ghost" className="flex-1 h-11 rounded-xl" onClick={onClose}>{t('holiday_modal.cancel')}</Button>
                        <Button
                        ...
                        >
                            {actionLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : t('holiday_modal.confirm')}
                        </Button>
                    </div>
                </div>
            </div>
