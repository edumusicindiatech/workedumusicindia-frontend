...
const AssignTaskModal = ({ isOpen, onClose, employeeId, initialData, onSuccess }) => {
    ...
    // --- Pre-fill the form when cloned data is passed ---
    useEffect(() => {
        if (isOpen) {
            if (initialData) {
                ...
                <div className="p-6 space-y-6 overflow-y-auto flex-1 custom-scrollbar">
                    ...
                    <div className="p-5 bg-background border border-border rounded-2xl shadow-sm space-y-4 relative overflow-hidden">
                        ...
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 relative z-10">
                            ...
                            {/* --- THE FIX: Smart dynamic fallback anchor tag --- */}
                            <a
                                href={
                                    taskForm.latitude && taskForm.longitude
                                        ? `https://maps.google.com/?q=${taskForm.latitude},${taskForm.longitude}`
                                        : taskForm.location
                                            ? `https://maps.google.com/?q=${encodeURIComponent(taskForm.location)}`
