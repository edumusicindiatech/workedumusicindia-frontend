...
public class MainActivity extends BridgeActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        BroadcastReceiver callCancelReceiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                try {
                    String jsCode = "window.dispatchEvent(new CustomEvent('call_notification_cancelled'));";
                    injectWhenReady(jsCode, 20);
                } catch (Exception e) {
                    android.util.Log.e("VOIP_DEBUG", "Failed to inject cancel event: " + e.getMessage());
                }
            }
        };
        
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            registerReceiver(callCancelReceiver, new IntentFilter("com.workedumusic.app.CALL_CANCELLED"), Context.RECEIVER_NOT_EXPORTED);
        } else {
            registerReceiver(callCancelReceiver, new IntentFilter("com.workedumusic.app.CALL_CANCELLED"));
        }
    }
